<script lang="ts">
	...
	import { sundaneseLabels } from '$lib/models/labels';
	...
	async function handleCheck() {
		try {
			...
			await drawingStore.checkValidDrawing(blob, ApiSundaneseEndpoints.PRACTICE, sundaneseLabels);
		} catch (e) {
...
<svelte:head>
	<title>Practice - Sundanese Script</title>
	<meta
		name="description"
	...
	<div class="relative flex flex-1 flex-col items-center justify-center gap-6 md:flex-row">
		<div class="flex items-center justify-center text-center md:max-w-1/3 md:text-start">
			<h2 class="text-3xl font-extrabold text-gray-500">
				Write down the Sundanese characters you remember and check them!
			</h2>
		</div>
