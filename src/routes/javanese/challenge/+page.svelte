<script lang="ts">
	...
	let showHelp = false;
	let randomJavaneseScript = getRandomLabel();
	...
	async function handleCheck() {
		try {
			...
			await challengeStore.checkValidChallenge(randomJavaneseScript, blob, ApiEndpoints.CHALLENGE);
		} catch (e) {
	...
	function handleNextCharacter() {
		...
		if (drawStatus === 'success') {
			randomJavaneseScript = getRandomLabel();
		}
	}
	...
	function getRandomLabel() {
		const random = characterLabels[Math.floor(Math.random() * characterLabels.length)];
		return random.value;
	}
</script>

<svelte:head>
	<title>Challenge - Javanese Script</title>
	<meta name="description" content="Javanese Script Learning." />
	<meta
		name="keywords"
	...
	<div class="relative flex flex-1 flex-col items-center justify-center gap-6 md:flex-row">
		<div class="flex items-center justify-center text-center md:max-w-1/3 md:text-start">
			<h2 class="text-3xl font-extrabold text-gray-500">
				Write the Javanese script for “{randomJavaneseScript}”!
			</h2>
		</div>
